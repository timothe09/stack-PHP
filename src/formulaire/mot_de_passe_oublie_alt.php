...
// Si la colonne date_naissance n'existe pas, utiliser une méthode alternative
$alternative_method = !$date_naissance_exists;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['step']) && $_POST['step'] == 1) {
    ...
    if (empty($email)) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT id, email FROM connexion_games WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user) {
                ...
                if ($alternative_method) {
                    // Méthode alternative : passer directement à l'étape 3
                    $step = 3;
                    $_SESSION['reset_email'] = $email;
                    $_SESSION['reset_user_id'] = $user['id'];
                    $message = "Comme la fonctionnalité de vérification par date de naissance n'est pas disponible, vous pouvez directement définir un nouveau mot de passe. Notez que cette méthode est moins sécurisée.";
                } else {
                    // Méthode normale : passer à l'étape 2
                    $step = 2;
                    $_SESSION['reset_email'] = $email;
                    $_SESSION['reset_user_id'] = $user['id'];
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['step']) && $_POST['step'] == 2 && $date_naissance_exists) {
...
<body>
    <div class="container">
        ...
        <?php if ($alternative_method): ?>
            <div class="notice">
                <strong>Attention :</strong> La fonctionnalité de vérification par date de naissance n'est pas disponible car la colonne date_naissance n'existe pas dans la base de données. Veuillez exécuter le script update_database.php pour activer cette fonctionnalité.
            </div>
        <?php endif; ?>
        ...
        <div class="step-indicator">
            ...
            <?php endif; ?>
            <div class="step <?php echo $step == 3 ? 'active' : ''; ?>">Étape <?php echo $alternative_method ? '2' : '3'; ?>: Nouveau mot de passe</div>
        </div>
        
        <?php if (!empty($message)): ?>
            <div class="message <?php echo $success ? 'success' : ($alternative_method ? 'warning' : 'error'); ?>">
                <?php echo $message; ?>
            </div>
        <?php endif; ?>
        ...
        <?php elseif ($step == 2 && !$success && !$alternative_method): ?>
